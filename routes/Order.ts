...
router.post("/", async (req: any, res: any) => {
  ...
  await urunler.map((item: any) => {
    return ids.push(item.id) && adets.push(item.adet);
  });
  ...
  try {
    for (let i = 0; i < ids.length; i++) {
      order.Urunler.push(ids[i]) && order.Adetler.push(adets[i]);
    }
    await order.save();
  } catch (err) {
    console.log(err);
...
router.post("/iyzipay", async (req, res) => {
  ...
  order.Adetler.map((item: any) => {
    return adets.push(item);
  });
  ...
  for (let i = 0; i < urunler.length; i++) {
    mybasket.push({
      id: urunler[i]._id.valueOf(),
      name: urunler[i].Name,
      category1: "Collectibles",
      category2: "Accessories",
      itemType: Iyzipay.BASKET_ITEM_TYPE.PHYSICAL,
      price: Number(urunler[i].Price * adets[i]),
    });
  }
